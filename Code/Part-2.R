...
predplot +   geom_ribbon(aes(ymin = p.lwr, ymax = p.upr), alpha = 0.2) +   labs(title = "Low Plasma (=1) or Not low Plasma (=0) vs bmi",
...
ggplot(bmi_pred, aes(x = bmi, y = Dcook)) +
  ...
  facet_wrap(~lowplasma_01) +
  labs(title = "Cook's Distance vs BMI (Highlighting High Leverage & Large Residuals)",
